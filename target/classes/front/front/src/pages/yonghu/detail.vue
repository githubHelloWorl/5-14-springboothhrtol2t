	...
	<div class="detail-preview" :style='{"padding":"0 7%","margin":"40px auto","alignItems":"flex-start","flexWrap":"wrap","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
		<div class="attr" :style='{"padding":"0","margin":"0","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"55%","position":"relative","justifyContent":"space-between","order":"2"}'>
			...
			<div class="info" :style='{"width":"100%","padding":"0px","margin":"0","background":"#fff","flex":"1","order":"2"}'>
				...
				<div class="item" :style='{"padding":"0 10px","margin":"0 0 0px 0","borderColor":"#eee","background":"none","borderWidth":"0 0 1px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#999","textAlign":"right","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>年龄</div>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#333","borderRadius":"0px","background":"#f5340030","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('yonghu','好友申请')&&detail.id!=mid" @click="addFriendClick">好友申请</el-button>
				</div>
			</div>
		</div>
		...
		<div class="detail2" :style='{"border":"0px solid #ddd","padding":"0px","margin":"40px 0","borderRadius":"0px","background":"#fff","width":"100%","order":"50"}'>
			<div :style='{"border":"1px solid #f6f6f6","padding":"10px","boxShadow":"0px 6px 6px -6px #999","margin":"0 0 16px 0","borderRadius":"4px","background":"#fcfcfc"}'>
				<div :style='{"color":"#333","fontSize":"16px","fontWeight":"600"}'>简介</div>
			</div>
			<div :style='{"background":"#FFF"}' v-html="detail.jianjie"></div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      nextClick(page) {
		...
		addFriendClick() {
			this.$confirm('是否发送好友申请？')
				.then(_ => {
					this.$http.get('friend/page',{params: {
						uid: this.detail.id,
						...
						if(res.data&&res.data.code==0){
							if(res.data.data.list.length){
								if(res.data.data.list[0].type==1){
									this.$message.success('两人已经是好友了！')
									return false
								}else if(res.data.data.list[0].type==0){
									this.$message.success('已经发送了好友申请，请等待对方通过！')
									return false
								}
							}
							this.$http.get('friend/page',{params: {
								...
								if(res.data&&res.data.code==0){
									if(res.data.data.list.length){
										this.$message.success('对方已发送好友申请，请前往个人中心通过')
										return false
									}

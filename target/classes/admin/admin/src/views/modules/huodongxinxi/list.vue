<template>
	<div class="main-content" :style='{"color":"#666","padding":"0px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"1"}'>
					...
					<el-button class="btn18" v-if="isAuth('huodongxinxi','活动分类统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"12px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='true' prop="huodongzhaopian" width="200" label="活动照片">
						<template slot-scope="scope">
							<div v-if="scope.row.huodongzhaopian">
								<img v-if="scope.row.huodongzhaopian.substring(0,4)=='http'" :src="scope.row.huodongzhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.huodongzhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('huodongxinxi','报名')" @click="huodongbaomingCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="huodongfenleiChart1" style="width:100%;height:600px;"></div>
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var huodongfenleiChart1 = echarts.init(document.getElementById("huodongfenleiChart1"),'macarons');
        this.$http({
            url: "huodongxinxi/group/huodongfenlei",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                huodongfenleiChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    huodongfenleiChart1.resize();
                };
            }
        });
      })

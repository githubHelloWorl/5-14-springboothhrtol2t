...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 20px","margin":"20px 0 0","borderColor":"#ddd","alignItems":"center","flexWrap":"wrap","background":"none","borderWidth":"0 0 1px","display":"flex","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"100px","padding":"0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>社团名称：</div>
        <el-input v-model="formSearch.gangweimingcheng" placeholder="社团名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('ganbuzhaopin','新增')" :style='{"cursor":"pointer","border":"1px solid #ddd","padding":"0px 15px","margin":"0px 10px 0 0","outline":"none","color":"#333","borderRadius":"0px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/ganbuzhaopinAdd')"><i v-if="true" :style='{"color":"#333","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"width":"100%","padding":"0 0px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"0px solid #f6f6f6","width":"100%","padding":"0px","margin":"0 0px 0 0","overflow":"hidden","height":"240px"}' class="img-box">
				<img @click.stop="imgPreView(item.zhaopinfengmian)" :style='{"width":"100%","objectFit":"cover","display":"block","clipPath":"polygon(100% 88%, 88% 100%, 0% 100%, 0% 0%, 100% 0%)","height":"100%"}' v-if="item.zhaopinfengmian && item.zhaopinfengmian.substr(0,4)=='http'" :src="item.zhaopinfengmian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.zhaopinfengmian?item.zhaopinfengmian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","clipPath":"polygon(100% 88%, 88% 100%, 0% 100%, 0% 0%, 100% 0%)","height":"100%"}' v-else :src="baseUrl + (item.zhaopinfengmian?item.zhaopinfengmian.split(',')[0]:'')" class="image" />
			</div>
		    <div :style='{"width":"100%","padding":"0 0px","overflow":"hidden","alignItems":"center","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"55%","padding":"0 0px 0 0","flex":"1"}'>
				<div class="title">{{item.gangweimingcheng}}</div>
		      ...
		      <div class="desc" v-html="item.gangweixiangqing"></div>
		    </div>
		  </div>
		</div>
	</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`ganbuzhaopin/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/ganbuzhaopinDetail', query: params});
      },
